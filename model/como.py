...
class MultiScaleGatedSkipBlock(BaseModule):
    def __init__(self, in_channels):
        super(MultiScaleGatedSkipBlock, self).__init__()
        self.gate_1x1 = torch.nn.Conv2d(in_channels, in_channels//4, 1)  
        self.gate_3x3 = torch.nn.Conv2d(in_channels, in_channels//4, 3, padding=1)  
        self.gate_5x5 = torch.nn.Conv2d(in_channels, in_channels//4, 5, padding=2)  
        self.gate_global = torch.nn.AdaptiveAvgPool2d(1)  
        self.gate_global_conv = torch.nn.Conv2d(in_channels, in_channels//4, 1)
        
        self.fusion = torch.nn.Conv2d(in_channels, in_channels, 1)
        self.sigmoid = torch.nn.Sigmoid()

    def forward(self, x):
        B, C, H, W = x.shape
        
        gate_1 = self.gate_1x1(x)  # [B, C//4, H, W]
        gate_3 = self.gate_3x3(x)  # [B, C//4, H, W]  
        gate_5 = self.gate_5x5(x)  # [B, C//4, H, W]
        
        gate_g = self.gate_global(x)  # [B, C, 1, 1]
        gate_g = self.gate_global_conv(gate_g)  # [B, C//4, 1, 1]
        gate_g = gate_g.expand(-1, -1, H, W)  # [B, C//4, H, W]
        
        multi_scale_gate = torch.cat([gate_1, gate_3, gate_5, gate_g], dim=1)  # [B, C, H, W]
        
        gate = self.sigmoid(self.fusion(multi_scale_gate))
        return x * gate
...
class GradLogPEstimator2d(BaseModule):
    def __init__(self, dim, dim_mults=(1, 2, 4), groups=8,
        ...
        # 为每个跳跃连接层添加多尺度门控模块
        skip_dims = [dim * mult for mult in reversed(dim_mults[1:])]  # 跳跃连接的维度
        self.gated_skip_blocks = torch.nn.ModuleList([
            MultiScaleGatedSkipBlock(skip_dim) for skip_dim in skip_dims
        ])
    ...
    def forward(self, x, mask, mu, t, spk=None):
        ...
        # 解码器部分 - 在跳跃连接处应用多尺度门控
        for stage_idx, (resnet1, resnet2, attn, upsample) in enumerate(self.ups):
            mask_up = masks.pop()
            hidden = hiddens.pop()
            
            # 对跳跃连接应用多尺度门控增强
            if stage_idx < len(self.gated_skip_blocks):
                hidden = self.gated_skip_blocks[stage_idx](hidden)
